...
## ADR-0007 — Generate occasion pages, but only two of the eight

**Date:** 2026-09-07
**Status:** accepted

**Context.** Every one of the 66 set menus in `menu-data.js` carries occasion
tags, from a closed vocabulary of eight that `menu-data.test.js` pins. Until
now nothing read them except the explorer's filter on `/menu/`. The category
pages added in September 2026 (`/menu/tiffin|lunch|dinner/`) are crawlable but
poorly aimed: nobody searches "dinner catering menu". The commercial queries
are occasion-shaped — "housewarming catering menu Chennai".

Measured across the whole dataset on 7 Sep 2026, before choosing:

| Occasion | Sets | Dish entries | Conflict |
|---|---|---|---|
| wedding | 33 | 612 | `/services/` title targets wedding catering |
| reception | 33 | 640 | shares 26 of 40 sets with wedding — 65% overlap |
| housewarming | 18 | 174 | none |
| seemantham | 14 | 128 | named only in the `/services/` description |
| birthday | 14 | 184 | none |
| corporate | 11 | 98 | `/corporate/` is a dedicated page |
| puja | 5 | 61 | thin |
| temple | 4 | 56 | thin; 29% overlap with puja |

**Decision.** Build `/menu/housewarming/` and `/menu/seemantham/` only,
generated by the same `tools/build-menu-pages.mjs` that produces the category
pages, and committed the same way. Both are self-canonical.

Two, not eight, because all eight would ship a `/menu/reception/` that is 65%
the same page as `/menu/wedding/`, a `/menu/corporate/` competing with the
`/corporate/` page for the same query, and two pages carrying four and five
sets. Housewarming and seemantham are the only occasions with no page already
aimed at them, and they are substantial enough to stand alone.

**Consequence.** Five generated pages instead of three, from one template
module. `renderSet` now takes a heading level and the stylesheet keys off
`.set-name` / `.set-group` rather than `.set h2` / `.set h3`, because occasion
pages group sets under a per-category heading. `site/menu-occasion-pages.test.js`
holds the pages to the data in both directions — every tagged set present, and
no untagged set present, which is the half that catches a broken filter.

Both pages repeat dish text that already appears on a category page, and share
8 of their 24 sets with each other. That near-duplication is accepted, not
solved: what separates the pages is the authored per-occasion intro and a link
from each section to the full category page. **Do not "fix" it with a
cross-canonical** — canonicalising these to `/menu/tiffin/` would deindex them
and there would be no reason to have built them.

**The gate on the other six.** Revisit only after these two have been live a
month and Search Console shows them indexed and drawing impressions. If they
are, `/menu/birthday/` is next. `/menu/wedding/` must be a single page covering
wedding and reception together, never two. `/menu/corporate/` should probably
never exist; link the 11 corporate sets from `/corporate/` instead. If these
two are *not* indexed, the answer is not six more pages — it is finding out why
these failed.

**Not addressed.** Occasion names in Tamil. `menu-data.js` carries `tamil` per
category but nothing per occasion, and which vernacular term the customers
actually use — griha pravesam, paal kaachal, valaikaappu — is the owner's call.
The pages use English names until that is confirmed.

---

## ADR-0008 — Delete `site/netlify.toml` and `site/.htaccess`
...
## ADR-0009 — Google Analytics 4 on every page, with the config half in a file
...
less. ADR-0008 noted that duplication; it is still the real problem here.
...
## ADR-0010 — Sync the shared topbar and nav in place, rather than templating pages
...
ADR-0008 named this class of duplication as the real problem and left it open.
...
**Extended to `<head>`, same day. This closes the CSP duplication ADR-0008
...
`head-twitter-image` (`twitter:image` and its alt text). ADR-0008 counted the
CSP in eleven places and called the ten per-page copies "the real duplication";
they are now one file plus `site/_headers`. ADR-0009 paid for that duplication
...
**Context.** The 13 Sep page audit found `/services/` trying to rank for six
occasion keywords with about 40 words each, and seven identical "Enquire on
WhatsApp" buttons. Competitors rank with one page per occasion. The conversion
goal is a quote request or a call. ADR-0007 already rules that wedding and
reception are one page and that `/corporate/` owns corporate catering.

**Decision.**
- Build occasion service pages at `/services/<occasion>/`, generated by
  `tools/service-page-template.mjs` through `npm run build:menu`, so their sets
  come from `menu-data.js`. Phase 1 builds two: `wedding-reception-catering`
  and `puja-homam-catering`.
- **Menus, after the owner's review (13 Sep 2026):** the wedding page features
  no particular menu. It offers every set menu, as a tile per meal linking to
  the full menu page and a grouped dropdown in the quote form. The puja page
  shows no menu and its form asks for none, until the owner supplies a sattvic
  menu to add to `menu-data.js`. The kitchen photo stays off both pages.
- Each page offers three actions in a fixed order of weight: **Get a quote**
  (the form), **Call**, and a WhatsApp text link. Links name their position
  with `data-cta-position` for GA4.
- The quote form has no named fields and composes a WhatsApp message in the
  browser (`site/quote-form.js`). Nothing is stored or submitted to the site,
  consistent with ADR-0006. The message ends with a ref line naming the page,
  because GA4 cannot see whether a WhatsApp message was actually sent.
- Generated service pages copy their chrome from `site/corporate/index.html`,
  whose nav marks no link as current, via `chromeSourceFor()` in
  `tools/sync-chrome.mjs`.
- Keyword ownership: a service page owns "<occasion> catering in Chennai"; a
  `/menu/<occasion>/` page owns "<occasion> catering menu". The `/services/`
  hub stops targeting any single occasion.
...
`setsByCategory` accepts several occasion keys. An interim version of the puja
page filtered the puja and temple sets to sattvic ones (a `NOT_SATTVIC` dish
list), which left a single set. The owner's review replaced that with no menu
at all, so the filter was removed with it.
