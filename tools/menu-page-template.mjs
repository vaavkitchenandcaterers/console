...
//
// Escaping and dish counting come from site/menu-format.js — the same module
// the browser loads — so a page cannot render a dish differently from the way
// the menu explorer renders it. tools/ already reads site/ (menu-data.js,
// menu/index.html); this is the same direction.
...
function buildHead(sourceHead, url, meta) {
  let h = sourceHead
    ...
    .replace(/<meta name="twitter:title" content="[^"]*">/, `<meta name="twitter:title" content="${escapeHtml(meta.ogTitle)}">`)
    .replace(/<meta name="twitter:description" content="[^"]*">/, `<meta name="twitter:description" content="${escapeHtml(meta.ogDescription)}">`)
...
function renderSet(m, level = 2) {
  ...
  parts.push(`        <h${level} class="set-name">${escapeHtml(m.name)}</h${level}>`);
  ...
  for (const [label, items] of m.groups || []) {
    ...
    if (!single) parts.push(`        <h${level + 1} class="set-group">${escapeHtml(label)}</h${level + 1}>`);
...
export function renderCategoryPage(catKey, data, chrome) {
  ...
  const main = [
    ...
    `    <nav class="breadcrumb" aria-label="Breadcrumb"><a href="/">Home</a> <span aria-hidden="true">/</span> <a href="/menu/">Menu &amp; Packages</a> <span aria-hidden="true">/</span> <span aria-current="page">${escapeHtml(data.label)}</span></nav>`,
    '    <div class="sec-head">',
    `      <p class="eyebrow">${escapeHtml(data.label)} &middot; ${escapeHtml(data.tamil)}</p>`,
  ...
  const bottom = withoutDataset(chrome.bottom);
  ...
  return assertNoDataset(html, catKey);
}

/**
 * Drop the dataset from the bottom chrome. Only the /menu/ explorer reads
 * window.VAAV_MENUS, and it bails on any page without #catTabs. No generated
 * page has one, and every dish is already there as static HTML, so
 * menu-data.js would be 21 KB fetched and parsed for nothing. /script.js
...
/**
 * Guard the strip above. It matches the tag as menu/index.html writes it today;
 * reformat that tag and the strip silently stops firing, putting 21 KB back on
 * every generated page with nothing to notice. Break the build instead.
...
export function renderOccasionPage(occKey, menus, chrome) {
  ...
  const main = [
    '<main id="main">',
    '  <section class="wrap sec">',
    `    <nav class="breadcrumb" aria-label="Breadcrumb"><a href="/">Home</a> <span aria-hidden="true">/</span> <a href="/menu/">Menu &amp; Packages</a> <span aria-hidden="true">/</span> <span aria-current="page">${escapeHtml(meta.label)}</span></nav>`,
    '    <div class="sec-head">',
  ...
  return assertNoDataset(html, occKey);
}
